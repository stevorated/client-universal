...
export default function EventMainCard(props) {
  const { handleAction, event, myId } = props
  const propsToPass = {
    handleAction,
    event,
    myId
  ...
  return (
    <FlatCardStatic className="px-0 pt-3 mt-3">
      ...
      <div className="mx-3" style={{ minHeight: '200px' }}>
        <p className="text-left lead mb-0" style={{ fontWeight: '900' }}>
          {props.title}
        </p>
        <hr
          className="noPadding"
        ...
        />
        {props.feedMode ? (
          <NextUpFeedContainer {...propsToPass} />
        ) : (
          <NextUpBoardContainer {...propsToPass} />
        )}
        <hr style={{ margin: '0' }} />
        <p className="text-left lead mt-4 mb-0" style={{ fontWeight: '900' }}>
          Create an Event
        </p>
        <hr
          className="noPadding"

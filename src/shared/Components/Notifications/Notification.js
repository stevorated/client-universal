...
import { timeAgo, elevation, orange } from "../../Utils"
...
export default function Notification(props) {
  ...
  const renderNotification = () => {
    ...
    const FromNameLink = () => (
      <Link style={{ fontWeight: "900" }} to={`/profile/${from.id}`}>
        {fromName}
      </Link>
    )
    switch (type) {
        ...
        if (lastAction === "Unlike-Post") {
          ...
          return (
            <CardText>
              <FromNameLink />
              {text} <br />
              <span className="lo-text">{timeAgo(Date.now(), createdAt)}</span>
            </CardText>
          )
        } else {
      ...
      case "Event":
        ...
        if (lastAction === "Create-Event") {
          const text = ` created a new event "${
            event.name
          }" that's on ${eventTimeString} Go Have a look inside.`
          return (
            <CardText>
              <FromNameLink />
              {text}
          ...
          const text = ` deleted an event "${
            event.name
          }" that's was supose to be on ${eventTimeString}`
          return (
            <CardText>
              <FromNameLink />
              {text}
      ...
      case "EventFollowers":
        ...
        if (lastAction === "Unfollow-Event") {
          const text = ` unfollowed ${to.fname} ${to.lname}'s event "${
            event.name
          }" that's on ${timeString}`
          return (
            <CardText>
              <FromNameLink />
              {text}
          ...
          const text = ` followed ${to.fname} ${to.lname}'s event "${
            event.name
          }" thats's on ${timeString}`
          return (
            <CardText>
              <FromNameLink />
              {text}
      ...
      case "Comment":
        if (lastAction === "Create-Comment") {
          const text = ` commented "${comment.body}" on ${to.fname} ${
            to.lname
          }'s post `
          return (
            <CardText>
          ...
          const text = `uncommented "${comment.body}" on ${to.fname} ${
            to.lname
          }'s post `
          return (
            <CardText>
      ...
      case "Profile":
        const text = `new user joined: ${from.fname} ${
          from.lname
        }, wanne go greet him with your wisdom?`
        return (
          <CardText>
            {text}
            <br />

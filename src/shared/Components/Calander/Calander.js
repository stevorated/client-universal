...
import { dayOfWeek, makeMonthsArray, daysArray, mediaQueries, elevation } from '../../Utils'
...
export default function Calander(props) {
  ...
  return (
    <FlatCardStatic className={props.className}>
      ...
      {eventsInFocus.length > 0 && (
        <div className="animated slideInDown mt-4">
          ...
          : (
            <CalanderDayEvents events={eventsInFocus} />
            )}
        </div>
      )}
    </FlatCardStatic>
